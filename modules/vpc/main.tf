...
resource "aws_route_table_association" "private" {
  ...
  route_table_id = var.create_nat_gateway ? aws_route_table.private[count.index].id : aws_route_table.private[count.index].id
}
...
resource "aws_vpc_endpoint_route_table_association" "private_s3" {
  count           = var.create_s3_endpoint ? length(var.private_subnet_cidrs) : 0
